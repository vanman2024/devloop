/**
 * Test script for AI-powered document generation (v2 endpoint)
 */
...
async function testAIDocumentGeneration() {
  ...
  console.log("Testing AI-powered document generation (v2 endpoint):");
  console.log(JSON.stringify(documentRequest, null, 2));

  try {
    console.log("Sending request to AI endpoint...");
    const response = await fetch('http://localhost:3002/api/documents/generate-with-ai-v2', {
      method: 'POST',
      headers: {
