...
start_server() {
  ...
  # Check if required modules are installed in UI directory
  if [ ! -f "ui/node_modules/express/package.json" ] || 
     [ ! -f "ui/node_modules/ws/package.json" ]; then
    echo "Required dependencies missing. Please run 'cd ui && npm install' first."
    exit 1
  fi
